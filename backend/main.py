...
def register_user(user: schemas.UserCreate, db: Session = Depends(db_mod.get_db)):
    ...
    new_user = db_mod.User(username=user.username, hashed_password=hashed_password, is_active=True)
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
    return new_user
...
async def send_message(chat_id: int, message: schemas.MessageCreate, current_user: db_mod.User = Depends(auth.get_current_user), db: Session = Depends(db_mod.get_db)):
          ...
          try:
                        ...
                        assistant_msg = db_mod.ChatMessage(chat_id=chat_id, role="assistant", content="Generated your image:", image_url=image_url)
                        db.add(assistant_msg)
                        img_metadata = db_mod.ImageMetadata(user_id=current_user.id, prompt=message.content, file_path=local_path, url=image_url)
                        db.add(img_metadata)
                        db.commit()
